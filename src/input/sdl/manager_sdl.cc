/*
   $Id: manager_sdl.cc,v 1.1 2003/07/18 15:16:09 gnurou Exp $
...
void input_manager_update()
{
    ...
    while (SDL_PollEvent(&event))
    {
        switch (event.type)
        {
            case SDL_KEYDOWN:
            {
                keyboard_event ke (keyboard_event::KEY_PUSHED, sdl_key_trans[event.key.keysym.sym], event.key.keysym.unicode);
                manager::raise_event (ke);
                break;
            }
            case SDL_KEYUP:
            {
                keyboard_event ke (keyboard_event::KEY_RELEASED, sdl_key_trans[event.key.keysym.sym], event.key.keysym.unicode);
                manager::raise_event (ke);
                break;
            }
